...
Metro İstanbul için ilk kullanıcı odaklı dilim, M1A ve M4'ün ana yönleri ile M2'nin Hacıosman ve Seyrantepe kollarındaki ilk/son hareket özetidir. Bu özet yalnız bakım sırasında resmî sayfada seçilen gün için alınır; kaynak geçerlilik aralığı yayımlamadığından uygulama bunu "bugün kesin geçerli" diye sunmaz. Yenilemek için:

```bash
npm run data:build-metro-schedules
```

Komut varsayılan olarak yalnız `M1A`, `M2` ve `M4` için sekiz kontrollü kaynak sorgusu yapar. Tam istasyon bazlı saatler, durak eşlemeleri ayrıca doğrulanana kadar bu çıktıya eklenmez. Tek bir statik metro hattını yenilemek gerektiğinde `npm run data:build-metro -- --codes=M2` kullanılabilir; bu, hat/istasyon genel indekslerini değiştirmez.
