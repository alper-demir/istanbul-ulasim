...
Metro İstanbul için ilk kullanıcı odaklı dilim, M1A, M2 ve M4'ün ana yönlerinde ilk/son hareket özetidir. Bu özet yalnız bakım sırasında resmî sayfada seçilen gün için alınır; kaynak geçerlilik aralığı yayımlamadığından uygulama bunu "bugün kesin geçerli" diye sunmaz. Yenilemek için:

```bash
npm run data:build-metro-schedules
```

Komut varsayılan olarak yalnız `M1A`, `M2` ve `M4` için altı kontrollü kaynak sorgusu yapar. M2'nin Seyrantepe dalı ve tam istasyon bazlı saatler, durak eşlemeleri ayrıca doğrulanana kadar bu çıktıya eklenmez.
