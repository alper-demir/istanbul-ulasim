...
export function SchedulePanel({ dataset, selectedDirectionId, loading, error, unavailable, onRetry }: SchedulePanelProps) {
  ...
  return <div className="mt-3 space-y-3">
    ...
    <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--muted)]">{departureHeading}</p>{departures.length?<div className={`mt-2 grid gap-1.5 ${departureStopNames.length>1?'grid-cols-2':'grid-cols-4'}`}>{departures.slice(0,12).map((departure)=><span key={departure.journeyId} title={`${departure.stopName} · ${departure.time}${departure.marker ? ` · ${departure.marker}` : ''}`} className="rounded-md bg-[var(--surface-strong)] px-1.5 py-1.5 text-center"><span className="block text-xs font-extrabold">{departure.time}{departure.marker&&<sup className="ml-0.5 text-[8px] text-[var(--primary)]">{departure.marker}</sup>}</span>{departureStopNames.length>1&&<span className="mt-0.5 block truncate text-[8px] font-medium text-[var(--muted)]">{departure.stopName}</span>}</span>)}</div>:<p className="mt-2 text-[10px] text-[var(--muted)]">Bu gün türü için yayımlanmış kalkış bulunamadı.</p>}{departures.length>12&&<p className="mt-1.5 text-[9px] text-[var(--muted)]">İlk 12 hareket gösteriliyor · toplam {departures.length}</p>}</div>
