...
export function SchedulePanel({ dataset, selectedDirectionId, loading, error, unavailable, onRetry, showAll = false }: SchedulePanelProps) {
  ...
  if (unavailable || !dataset) return <div className="mt-3 rounded-lg border border-dashed border-[var(--border)] bg-[var(--surface-strong)] px-3 py-4 text-center"><CalendarClock className="mx-auto h-5 w-5 text-[var(--muted)]" /><p className="mt-2 text-xs font-bold">Bu hat için doğrulanmış sefer verisi yok</p><p className="mt-1 text-[10px] leading-relaxed text-[var(--muted)]">Resmî kaynak bu hat için kullanılabilir bir tarife snapshot’ı sunmadı. Güzergâh verisi etkilenmez.</p></div>;
